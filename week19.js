...
function makeTwo() {
  fetch("https://emojihub.yurace.pro/api/random/group/face-positive")
    ...
    .then((data) => {
      console.log(data);
      const newP = document.getElementById("result2");
      let p = document.createElement("p");
      p.textContent = data.htmlCode[0];
      newP.appendChild(p);
    })
    .catch((err) => {
      console.log("Ошибка. Запрос не выполнен: ", err);
...
function makeNine() {
  fetch("https://api.agify.io/?name=vadim")
    ...
    .then((data) => {
      ...
      resultElement.textContent = JSON.stringify(data); //Ваш код
    })
    .catch((error) => {
...
function makeTen() {
  fetch("https://dog.ceo/api/breeds/image/random")
    ...
    .then((data) => {
      ...
      imgElement.scr = data.message_url;
      resultElement.appendChild(imgElement); //Ваш код
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    }); //Ваш код
...
function makeEleven() {
  fetch("https://api.ipify.org?format=json")
    .then((response) => response.json())
    .then((data) => {
      document.getElementById("result11").innerHTML = data.ip; //Ваш код
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    });
...
function makeTwelve() {
  ...
  fetch(`http://ip-api.com/json/${ip}`)
    .then((response) => response.json())
    .then((data) => {
      console.log(`Страна вашего пребывания: ${data.country}`);
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    }); //Ваш код
...
function makeFourteen() {
  fetch("https://official-joke-api.appspot.com/random_joke")
    .then((response) => response.json())
    .then((data) => {
      console.log(data.setup, data.punchline); //Ваш код
    })
...
function makeSixteen() {
  ...
  fetch("https://jsonplaceholder.typicode.com/posts", {
    ...
    .then((response) => response.json())
    .then((data) => console.log(data))
    .catch((error) => console.error("Ошибка:", error)); //Ваш код
}
...
function makeTwentyOne() {
  fetch("https://jsonplaceholder.typicode.com/comments/1", {
    ...
    .then((response) => response.json())
    .then((json) => console.log(json)); //Ваш код
}
